...
> Also includes key concepts needed to understand the backend internals.
...
### 5. React + Vite for frontend — not Next.js
...
### 7. Only PostgreSQL runs in Docker during development
...
### 8. Full Docker Compose setup is a deployment-phase task
...
### 9. CI/CD with GitHub Actions — minimal pipeline
...
### 11. Service methods throw exceptions on "not found" — never return Optional
...
### 12. Custom exception classes — not generic RuntimeException
...
### 13. Password hashing in the service layer — not controller or entity
...
### 14. Validation errors handled in GlobalExceptionHandler — not per-controller
...
### 15. Role-based authorization: SecurityConfig for broad rules + @PreAuthorize for fine-grained rules
...
`@PreAuthorize("hasRole('ADMIN')")` handles fine-grained rules directly on controller methods — POST/PUT/DELETE books, GET all borrows.
...
### 16. Stateless API with JWT — no server-side sessions
...
### 17. JwtAuthenticationFilter identifies every request — never blocks
...
### 18. 401 vs 403 handled by separate mechanisms
...
### 19. No BorrowRequestDTO — bookId comes from URL path, userId from JWT
...
### 20. @PrePersist handles borrowDate, dueDate, and status — not the service
...
**The signature is the key insight.** If someone tampers with the payload, the signature breaks. The server detects it instantly. The secret key is never hardcoded — it lives in `application.properties` as `${JWT_SECRET}` and is injected via environment variables. The real value never touches the codebase.
...
Both read before either wrote. The library now has -1 copies. That is a data integrity failure that cannot be fixed after the fact.
...
Thread B now reads the updated value of 0 — not the stale 1. The check fails correctly. No double-borrow. No negative copies. Data integrity preserved.

**Why `@Transactional` is not optional here:** The lock exists only for the duration of the transaction. Without `@Transactional` on `borrowBook()`, there is no transaction — the lock is acquired and immediately released before any business logic runs. The entire `borrowBook()` method is the critical section.
...
| Strategy | Lock first, work second | Work first, detect conflict at write |
| Mechanism | `SELECT ... FOR UPDATE` | Version column, retry on conflict |
| Blocks other threads? | Yes — they wait | No — but may retry |
| Used here | ✅ Yes | ❌ No |
...
`@PrePersist` is a JPA lifecycle hook. It runs automatically just before a new entity is saved to the database for the first time. In `Borrow.java` it sets `borrowDate` (today), `dueDate` (today + 14 days), and `status` (ACTIVE) — the service never needs to think about these. This guarantees those fields are always correct regardless of how a `Borrow` is created, and keeps the service focused purely on business logic.

---

### Why DTOs — Not Entities Directly

Entities are the internal database representation. DTOs (Data Transfer Objects) are what travels over the wire in API responses. Returning entities directly exposes your entire database schema — including fields you never intended to share like hashed passwords and internal audit fields. DTOs give you full control over the API contract: what is exposed, how it is named, and what shape it takes. Changing an internal entity field doesn't accidentally break the API.
